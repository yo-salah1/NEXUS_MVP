...
NEXUS Backend API
Port: 8080
Connects Frontend ↔ SER API (port 8001) ↔ LLM
"""
...
FER_API_URL = "http://localhost:8002"
...
class ChatRequest(BaseModel):
    ...
    emotion: Optional[str] = None        # من الـ SER لو موجود
...
async def chat(req: ChatRequest):
    ...
    # Get LLM response (with emotion context if available)
    reply = await get_llm_response(req.message, session_id, req.emotion)
...
# ==========================
# ROUTES — VOICE (SER Integration)
# ==========================
@app.post("/api/voice/process")
async def voice_process(req: VoiceProcessRequest):
    """Text-based voice processing (fallback when no audio file)"""
    session_id = req.sessionId or generate_session_id()
...
def get_all_history():
    ...
    if today_items:
        result.append({"separator": "Today", "items": today_items})
    if yesterday_items:
        result.append({"separator": "Yesterday", "items": yesterday_items})
    if older_items:
        result.append({"separator": "Older", "items": older_items})
